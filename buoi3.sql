...
alter table hoadon add constraint FK_hoadon_makh foreign key (makh)
 references khachhang(makh);
alter table hoadon add constraint FK_hoadon_manv foreign key (manv) 
references nhanvien(manv)
alter table cthd add constraint Fk_hoadon_sohd foreign key (sohd)
references hoadon(sohd)
alter table cthd add constraint FK_sanpham_masp foreign key (masp)
...
-- GOM NHOMS
select nuocsx, count(masp), sum(gia), avg(gia), max(gia), min(gia)
from sanpham
group by nuocsx
-- Với mỗi manv, cho biết tổng số lượng hóa đơn lập được?
select manv, count(sohd)
from hoadon
group by manv
-- với mỗi makh, cho biết tổng tiền chi tiêu của khách hàng đó
select makh, sum(trigia)
from hoadon 
group by makh
-- Cho biết mã khách hàng đã mua hàng trong tháng 7 2007
select makh
from hoadon
where nghd between '2007/7/1' and '2007/7/31'
